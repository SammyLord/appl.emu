document.addEventListener('DOMContentLoaded', () => {
    ...
    const cpu = {
        ...
        addr: {
            imm: () => { const addr = cpu.pc++; return addr; },
            zp: () => cpu.read(cpu.pc++),
            zpx: () => (cpu.read(cpu.pc++) + cpu.x) & 0xFF,
            zpy: () => (cpu.read(cpu.pc++) + cpu.y) & 0xFF,
            abs: () => cpu.read16(cpu.pc),
            absx: () => { const addr = cpu.read16(cpu.pc); cpu.pc += 2; return addr + cpu.x; },
            absy: () => { const addr = cpu.read16(cpu.pc); cpu.pc += 2; return addr + cpu.y; },
            ind: () => {
        ...
        op: {
            ...
            JSR: (addr) => { cpu.push16(cpu.pc - 1 + 2); cpu.pc = addr; },
            ...
            CMP: (addr) => { const val = cpu.read(addr); const res = cpu.a - val; cpu.setC(res >= 0); cpu.setZ(res); cpu.setN(res); },
            CPX: (addr) => { const val = cpu.read(addr); const res = cpu.x - val; cpu.setC(res >= 0); cpu.setZ(res); cpu.setN(res); },
            CPY: (addr) => { const val = cpu.read(addr); const res = cpu.y - val; cpu.setC(res >= 0); cpu.setZ(res); cpu.setN(res); },
            ...
            PHP: () => cpu.push(cpu.status | 0x10), PLP: () => { cpu.status = cpu.pop(); },
    ...
    cpu.instructions = {
        0x00: { execute: () => { /* BRK */ cpu.pc++; } }, // Simplified BRK
        ...
        0x40: { execute: () => { /* RTI */ } }, 0x41: { execute: () => cpu.op.EOR(cpu.addr.indx()) }, 0x45: { execute: () => cpu.op.EOR(cpu.addr.zp()) }, 0x46: { execute: () => cpu.op.LSR(cpu.addr.zp()) }, 0x48: { execute: () => cpu.op.PHA() }, 0x49: { execute: () => { cpu.op.EOR(cpu.addr.imm()); } }, 0x4A: { execute: () => cpu.op.LSR_A() }, 0x4C: { execute: () => { const addr = cpu.read16(cpu.pc); cpu.pc+=2; cpu.op.JMP(addr); } }, 0x4D: { execute: () => cpu.op.EOR(cpu.addr.abs()) }, 0x4E: { execute: () => cpu.op.LSR(cpu.addr.abs()) },
    ...
    function read(addr) {
        if (addr >= 0xD010 && addr <= 0xD013) {
            if (addr === 0xD010) { // Keyboard data
                if (keyboardBuffer.length > 0) {
                    ram[0xD011] &= ~0x80; // Clear strobe
                    return keyboardBuffer.shift();
                }
                return 0;
            }
            if (addr === 0xD011) { // Keyboard control
                if (keyboardBuffer.length > 0) {
                    return 0x80; // Strobe high
                }
                return 0;
            }
        }
    ...
    function write(addr, val) {
        if (addr === 0xD012) { // Display
            output.textContent += String.fromCharCode(val & 0x7F);
            return;
        }
        ram[addr] = val;
    }
    ...
    function handleKey(e) {
        ...
        if (char.length === 1) {
            ...
            appleCharCode = 0xDF; // Apple's back arrow
        }
